<div>
    <form action="{{ route('admin.products') }}">
        <button type="submit" class="btn btn-danger mb-3 mx-2">Back</button>
    </form>
    <div class="com_card mx-2">
        <h3 class="com_card_title mb-3">Edit Stock</h3>

        <label for="title" class="form_label">Current Stocks:{{$stocks}}</label>
        <div class="w-25">@include('livewire.admin.counter.edit-product-counter')</div>

        
        <form wire:submit="updateStocks">
            <button type="submit" class="btn-one mt-3">Update Stocks</button>
        </form>


        @if (session()->has('stock'))
            <div class="alert alert-success mt-2">{{ session('stock') }}</div>
        @endif
