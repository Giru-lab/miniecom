<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ __("Update your phone, address to where the ship will be delivered.") }}
        </p>
    </header>

    <form method="post" action="{{ route('profile.billing') }}" class="mt-6 space-y-6">
        @csrf
        @method('patch')
        ...
        <div class="flex items-center gap-4">
            ...
            @if (session('status') === 'profile-billing')
                <p
                    x-data="{ show: true }"
                    x-show="show"
                    x-transition
